 ...
 *
 * @param client        The client to use.
 * @param {import("./data/products.typing").products[]} productsList  The products list to use.
 * @private
 */
function validateShoppingCartTable(client, productsList) {
    ...
    client.elements("css selector", "table tr", function(result) {
        result.value.forEach(function(v, i) {
            ...
            client.elementIdElement(v.ELEMENT, "css selector", config.elements.removeItemButton, function(result) {
                assert.strictEqual(result.state, "success", `Le bouton 'X' pour le produit '${product.name
                }' doit être présent.`);
            });
            ...
            client.elementIdElement(v.ELEMENT, "css selector", config.elements.removeQuantityButton, function(result) {
                assert.strictEqual(result.state, "success", `Le bouton '-' pour le produit '${product.name
                }' doit être présent.`);
                client.elementIdEnabled(result.value.ELEMENT, function(result) {
                    const expectedValue = product.quantity > 1;
                    assert.strictEqual(result.value, expectedValue, `Le bouton '-' pour le produit '${product.name
    ...
    const quantity = productsList.reduce(function(sum, product) {
...
function validateTotalAmount(client, productsList) {
    ...
    const expectedTotalAmount = productsList.reduce(function(sum, product) {
        return sum + (product.quantity * product.price);
    }, 0);
    const expectedTotalAmountStr = utils.getFormattedPrice(expectedTotalAmount);
    client.verify.containsText(config.elements.totalAmount, expectedTotalAmountStr,
