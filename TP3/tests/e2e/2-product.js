...
const config = require("./data/config.json").product;
const productsList = require("./data/products.json");
const utils = require("./utils");

const expectedProduct = {};

/** @typedef {import("./1-products.js").Client} Client */
...
module.exports = {
    "Page d'un produit": function(client) {
        const randomId = utils.getRandomIntInclusive(1, productsList.length);
        const product = productsList[randomId - 1];
        ...
        client.elements("css selector", `${config.elements.features} > li`, function(result) {
            ...
            result.value.forEach(function(v, i) {
                client.elementIdText(v.ELEMENT, function(result) {
                    const feature = utils.stripHTMLTags(product.features[i]);
        ...
        const price = utils.getFormattedPrice(product.price);
        client.assert.containsText(config.elements.price, price, `Le prix du produit doit être '${price}$'.\n`);
    },
    "Produit invalide": /** @param {Client} client */ function(client) {
        const randomInvalidId = utils.getRandomIntInclusive(productsList.length + 1, 1000);
        client.url(`http://localhost:8000/${config.url}${randomInvalidId}`)
            ...
            .execute(function() {
                //this code is executed in the context of the browser. document is a valid global in the context of the browser.
    ...
    client.assert.containsText("#shopping-cart-quantity", "0", "La quantité associée à ce produit doit être de '0'.");*/
        ...
        const randomQuantity = utils.getRandomIntInclusive(1, 10);
    ...
    "La quantite ajoutee doit etre indiquee dans la liste de produits": `${function(client) {
        const productsConfig = require("./data/config.json").products;
        const productsList = require("./data/products-all-price-up.json");
